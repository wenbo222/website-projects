...
# Using ECB method of encryption
...
# Modified to allow for JS interaction
# Wenbo
...
def encrypt(text, key, rounds):
    ...
    ciphertext=reassemble(blocks, True)
    return ciphertext
...
def decrypt(ciphertext, key, rounds):
    ...
    blocks=get_blocks(ciphertext, True)
    keys=get_keys(key, rounds)
        ...
        for j in range(4):
            for i in range(4):
                if flag:
                    ...
                    if block[i][j] in ["01", "02", "03", "04", "05", "06", "07", "08", "09", "0a", "0b", "0c", "0d", "0e", "10"]: # Remove the effect of padding
                        return text
                    text+=chr(int(block[i][j], base=16))
    return text


def process_request(choice, message, rounds, raw_key):
    """
    process_request(string, string, int, string) -> string
    ...
    if choice=="encrypt":
        return encrypt(message, key, rounds)
    else:
        return decrypt(message, key, rounds)

if __name__=="__main__":
    if len(sys.argv)>1:
        # CLI Mode: choice, message, rounds, key
        choice=sys.argv[1].lower()
        message=sys.argv[2]
        rounds=int(sys.argv[3])
        raw_key=sys.argv[4]
        print(process_request(choice, message, rounds, raw_key))
    else:
        # Interactive Mode
        ...
        result=process_request(choice, message, rounds, temp)
        if choice=="encrypt":
