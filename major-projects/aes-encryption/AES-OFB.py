...
# Using OFB method of encryption
...
def rev(flag):
    ...
    If the boolean is True, returns False.
...
def encrypt(text, key, iv, rounds, flag):
    """
    encrypt(string/list, string, string, int, boolean) -> string

    If the boolean is True, returns the encrypted message.
    Otherwise, returns the decrypted message.
    """

    blocks=get_blocks(text, iv, rev(flag)) # Plaintext
    new_blocks=get_blocks(text, iv, rev(flag)) # Ciphertext
